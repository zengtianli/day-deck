...
// 站群访问闸（authgate）的客户端。
//
// 为什么需要它：本 app 的数据源 `day.tianli.cyou` **整站**挂着 authgate
// （2026-08-29 上线，`/` 与 `/api/` 两个 location 都 include authgate-protect）。
// 不带凭证去取 `/index.json`，URLSession 会跟着 302 走到登录页，
// 到手的是一个 **200 的 HTML**——JSON 解码失败，而失败原因完全指不出「你没登录」。
//
// ⚠ **不是**在闸上给这些 JSON 开洞。那等于把微信原文、诉讼材料、联系人对全网公开。
//   客户端持凭证 ≠ 内容变公开，这两件事别混。
//
// 本文件从 `~/Apps/ios/options-desk/Sources/Gate.swift` 移植（契约 6：移植不重写），
...
enum Gate {
    ...
    private static let service = "cyou.tianli.daydeck.gate"
